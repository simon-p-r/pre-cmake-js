...
internals.evalTemplate = (template, obj) => {
        ...
        if (~requiredPkgKeys.indexOf(pkgKey)) {
            ...
            if (pkgVal) {
                ...
                if (pkgKey === 'name') {
                    ...
                    if (pkgVal.module_path) {
                        merged.modulePath = pkgVal.module_path;
                    }
                    ...
                    if (pkgVal.library_suffix) {
                        merged.librarySuffix = pkgVal.library_suffix;
                    }
                }
...
exports.execute = (command, args, cb) => {
    ...
    if (mergedOpts.modulePath) {
        mergedOpts.modulePath = internals.evalTemplate(mergedOpts.modulePath, mergedOpts);
        mergedOpts.modulePath = Path.join(mergedOpts.directory, mergedOpts.modulePath);
    } else {
    ...
    if (!mergedOpts.librarySuffix) {
        mergedOpts.librarySuffix = '.node';
    }


    mergedOpts.module = Path.join(mergedOpts.modulePath, mergedOpts.moduleName + mergedOpts.librarySuffix);
    ...
    mergedOpts.hostedTarball = Url.resolve(mergedOpts.hostedPath, mergedOpts.version);
    mergedOpts.hostedTarball = `${mergedOpts.hostedTarball}/${mergedOpts.packageName}`;
